...
class RideEntity {
  ...
  startLong(startlong) {
    if (startlong < -180 || startlong > 180) {
      throw new Error(
        'ModelError: startLong should be not be greater than 180 or less'
          + ' than -180',
      );
    }
  ...
  endLat(endlat) {
    if (endlat < -90 || endlat > 90) {
      throw new Error(
        'ModelError: endlat should be not be greater than 90 or'
          + ' less'
          + ' than -90',
      );
    }
  ...
  endLong(endlong) {
    if (endlong < -180 || endlong > 180) {
      throw new Error(
        'ModelError: endLong should be not be greater than 180 or less'
          + ' than -180',
      );
    }
